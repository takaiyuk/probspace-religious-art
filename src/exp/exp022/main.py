...
from src.exp.exp022.config import InputPath, ModelConfig, OutputPath
from src.utils import AverageMeter, CutMix, DefaultLogger, Jbl, Logger, fix_seed
    ...
    def _train_epoch(self, train_loader, model, criterion, optimizer, scheduler, epoch):
        ...
        for _ in range(self.cfg.params.num_aug):
            for _, image_label_dict in enumerate(train_loader):
                cm = CutMix()
                images_cutmix, labels_cutmix = cm.augment(image_label_dict)
                images = images_cutmix.to(self.cfg.basic.device)
                # labels = image_label_dict.get("label").to(self.cfg.basic.device)
                batch_size = labels_cutmix.labels_a.size(0)

                y_preds = model(images)
                # loss = criterion(y_preds, labels)
                loss = cm.loss(criterion, y_preds, labels_cutmix)
