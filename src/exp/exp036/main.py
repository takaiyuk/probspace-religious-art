...
from src.exp.exp036.config import InputPath, ModelConfig, OutputPath
...
class StackingRunner(BaseRunner):
    def _train(self, train: pd.DataFrame, n_fold: int):
        ...
        for run_name in self.cfg.stacked_features:
            ...
            preds.append(pred)
            scores.append(model_dict["best_score"])
        torch.save(
            {
                "preds": preds,
                "config": self.cfg,
        ...
        preds = np.array(
            [np.argmax(p, axis=1) for p in preds]
        ).T  # shape is (num_records, num_features)
        df_preds = pd.DataFrame(
            preds, columns=[f"preds_{i}" for i in range(preds.shape[1])]
        )
        valid_folds = pd.concat((valid_folds, df_preds), axis=1)
        valid_folds = valid_folds.assign(preds=df_preds.mode(axis=1)[0])
        return valid_folds
...
class StackingInferenceRunner(BaseRunner):
    def _test(self, test: pd.DataFrame, n_fold: int):
        ...
        df_preds = df_preds.assign(preds=df_preds.mode(axis=1)[0])
        return df_preds["preds"].values
    ...
    def run_cv(
        ...
        df_preds = pd.DataFrame(
            np.array(preds).T, columns=[f"preds_{i}" for i in range(len(preds))]
        )
        df_preds = df_preds.assign(preds=df_preds.mode(axis=1)[0])
        preds_mean = df_preds["preds"].values
