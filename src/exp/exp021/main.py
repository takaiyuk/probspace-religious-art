...
from src.exp.exp021.config import InputPath, ModelConfig, OutputPath
...
class StackingInferenceRunner(BaseRunner):
    ...
    def _test(self, test: pd.DataFrame, n_fold: int, build_models: Dict[str, Any]):
        self.logger.info(f"fold: {n_fold}")
        ...
        for run_name in tqdm(
            self.cfg.stacked_features,
            total=len(self.cfg.stacked_features),
            desc=f"[{self.__class__.__name__} fold{n_fold}]",
        ):
            model = build_models[run_name]
            model_state = torch.load(f"{OutputPath.model}/{run_name}_{n_fold}.pth")[
                "model"
            ]
    ...
    def run_cv(
        ...
        build_models: Optional[Dict[str, Any]] = None,
    ) -> None:
        # oof_df = pd.DataFrame()
        preds: List[np.array] = []
        for n_fold in range(self.cfg.kfold.number):
            preds_fold = self._test(test, n_fold, build_models)
...
def main():
    ...
    for run_name in model_config.stacked_features:
        model_name = getattr(model_config.params, f"model_name_{run_name}")
        build_models.update({run_name: build_model(model_config, model_name)})
    ...
    StackingInferenceRunner(model_config, logger).run_cv(build_models=build_models)
